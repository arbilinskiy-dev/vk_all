---
name: tester
description: Комплексное тестирование бэкенда и фронтенда. Модульная архитектура — каждая процедура в отдельном файле. Триггеры — «протестируй», «покрой тестами», «проверь что работает», «smoke-тест», «тест API», «проверь импорты», «тест после рефакторинга», «тест бизнес-логики», «интеграционные тесты», «проверь авторизацию», «контрактный тест», «e2e тест», «playwright», «визуальный тест», «тест UI».
---
...
- «autospec», «контракт моков», «mock contract», «тесты не ловят баги»
...
| «autospec», «контракт моков», «mock contract», «тесты не ловят» | `procedures/13_mock_contract.md` | 🔴 P0 |
| Первый запуск, нет `tests/` | `procedures/00_setup.md` | — |

---

## Requirement-Driven Tests (обязательно для новых фич)

Перед написанием тестов для нового функционала — **сформулируй ТРЕБОВАНИЕ** одним предложением:

```
Формула: «При [условие] пользователь видит [что] в [где]»
```

**Первый тест ВСЕГДА проверяет это требование напрямую:**

| ❌ Тест реализации (ловит только поломку кода) | ✅ Тест требования (ловит и промахи проектирования) |
|---|---|
| «чекбокс includeEnvToken рендерится» | «при настроенном ENV-токене, список системных аккаунтов содержит запись с (ENV)» |
| «поле notes присутствует в ответе» | «getProjectDetails возвращает notes, а getInitialData — нет» |
| «кнопка Удалить вызывает onDelete» | «после удаления элемента, список обновляется и элемент исчезает» |

**Порядок:**
1. Напиши контрактный тест (от требования) — он ДОЛЖЕН проходить
2. Только потом покрывай детали реализации (toggle, handlers, edge cases)
3. Если контрактный тест не проходит — значит реализация неверна, даже если все unit-тесты зелёные

> Этот подход ловит ситуации, когда код работает корректно, но решает не ту задачу.
...
СЛОЙ 2 — Unit-тесты (~5 мин, 3 параллельных саб-агента):
  ...
  ⚡ Саб-агент C [backend]: Mock-contract → autospec на зависимости → пишет tests/<модуль>/ → pytest
  → Чекпоинт: vitest ✅/❌ + pytest ✅/❌ + mock-contract ✅/❌
...
| 12_model_field | backend | tests/<модуль>/ | 07, 11, 06, 13 |
| 13_mock_contract | backend | tests/<модуль>/ | 07, 11, 06, 12 |
...
```
 ...
 6. � Mock-Contract (autospec)       — 1-3 мин
 7. �🟡 Unit-тесты API-слоя           — 1-3 мин
...
10. **autospec=True ОБЯЗАТЕЛЕН** для каждой мокированной зависимости в @patch. MagicMock без spec молча глотает несуществующие методы и kwargs → тесты зелёные, runtime падает
11. При покрытии модуля тестами — ВСЕГДА добавлять класс `Test<Module>ContractSafety` с autospec на все мокированные зависимости
