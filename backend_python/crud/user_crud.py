...
def update_users(db: Session, users_data: List[schemas.User]):
    ...
    Пароли хранятся в открытом виде для отображения админу.
    """
        ...
        if user_data.id.startswith('new-'):
            ...
            if user_data.full_name and user_data.username and user_data.password:
                new_db_user = models.User(
                    ...
                    password=user_data.password  # Сохраняем пароль как есть
                )
                db.add(new_db_user)
        else:
            ...
            if db_user:
                ...
                if user_data.password:
                    db_user.password = user_data.password  # Сохраняем пароль как есть
